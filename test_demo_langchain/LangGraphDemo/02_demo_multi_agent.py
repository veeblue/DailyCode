...
@tool
def python_repl_with_plot_save(
        code: Annotated[str, "要执行以生成图表的Python代码。图表将自动保存为PNG文件。"],
):
    """使用这个工具来执行Python代码并保存图表。
    如果代码包含matplotlib图表，会自动保存为PNG文件。
    如果你想查看某个值的输出，应该使用print(...)。"""

    try:
        # 修改代码，将plt.show()替换为plt.savefig()
...
''')
        ...
        if os.path.exists(chart_path):
            chart_info = f"\n✅ 图表已成功生成并保存为: {chart_path}"

    except BaseException as e:
        return f"执行失败。错误: {repr(e)}"

    result_str = f"成功执行:\n```python\n{modified_code}\n```\nStdout: {result}{chart_info}"
    return result_str + "\n\n如果你已完成所有任务，请回复FINAL ANSWER。"
